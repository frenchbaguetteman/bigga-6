...
void Drive::pid_odom_set(std::vector<odom> imovements) {
  if (imovements.empty()) {
    printf("No odom movements were provided!\n");
    return;
  }
...
void Drive::pid_odom_ptp_set(odom imovement, bool slew_on) {
  ...
  if (odom_feedback_get() != PID_FEEDBACK)
    build_odom_reference_states({active_odom_motion}, odom_pose_get());
  else
...
void Drive::raw_pid_odom_pp_set(std::vector<odom> imovements, bool slew_on) {
  ...
  if (odom_feedback_get() != PID_FEEDBACK)
    build_odom_reference_states(pp_movements, odom_pose_get());
  else
    reset_odom_reference();
