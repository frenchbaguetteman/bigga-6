...
std::vector<pose> Drive::find_point_to_face(pose current, pose target, drive_directions dir, bool set_global) {
  ...
  // Use the same atan2-based frame as the rest of EZ odom math so the
  // point-to-face line stays in the correct quadrant near the target.
  double angle = util::absolute_angle_to_point(target, current);
  ...
  // printf("pft1(%.2f, %.2f, %.2f)   ptf2(%.2f, %.2f, %.2f)      angle: %.2f\n", point_to_face[0].x, point_to_face[0].y, point_to_face[0].theta, point_to_face[1].x, point_to_face[1].y, point_to_face[1].theta, angle);
